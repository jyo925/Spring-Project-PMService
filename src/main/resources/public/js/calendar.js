$(function() {
	...
	$('#calendar').fullCalendar({
				...
				navLigYnks : true, // can click day/week names to navigate
									// views
				editable : true,
				...
				eventClick : function(calEvent, jsEvent, view) {
					...
					$('#description', '#dialog-event').text(calEvent.description);
					$('#place', '#dialog-event').val(calEvent.place);
					$('#eventId', '#dialog-event').val(calEvent.eventId);
					if (calEvent.allDay === true) {
						$('#allDay').prop("checked", true);
					} else {
						$('#allDay').prop("checked", false);
					};
					let select = $('option', '#type');
					for (let i = 0; i < select.length; i++) {
					    if (select[i].value === calEvent.type) select[i].selected = true;
					}
					let select1 = $('option', '#project');
	...
	let textList1 = $('.textList');
	
	$('.memb').on('click', function() {
		let regexp = new RegExp($(this).text(), "gi");
		let replace = new RegExp(" " + $(this).text(), "gi");
		if (textList1.text() == "") {
			textList1.text(" " + $(this).text());
			$(this).removeClass("btn-light");
		} else if (textList1.text().search(regexp) != -1) {
			textList1.text(textList1.text().replace(replace, ""));
			$(this).addClass("btn-light");

		} else {
			textList1.text(textList1.text() + " " + $(this).text());
			$(this).removeClass("btn-light");
		}
	...
	let editForm = $("#EditEvent");
	$('.button-delete').on("click", function(e) {
		e.preventDefault();
		editForm.attr("action", "/deleteEvent");
		editForm.submit();
	});

	//AIzaSyDi_TYmdZHmwljgkQ9yOyGhcj8_lXthkB4 google api key
	//dragon.rain.kim.9@gmail.com
	
	
	/////////////////////event statistic part///////////////////////////////////
	...
	function pieChart() {
	  ...
	  function init() {
	    ...
	    chartRadius = Math.min( canvasWidth, canvasHeight ) / 1.5 * ( chartSizePercent / 100 );
